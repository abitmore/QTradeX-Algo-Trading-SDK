...
Methods:
    ...
    create_order(self, symbol, side, order_type, amount, price):
        Creates a limit order on the exchange.

    cancel_order(self, order_id, symbol):
        Cancels a single open order.

    cancel_orders(self, order_ids, symbol):
        Cancels multiple open orders.

    cancel_all_orders(self, symbol):
        Cancels all open orders for a given symbol asynchronously.

    fetch_open_order(self, order_id, symbol=None):
        Fetches details of a specific open order.

    fetch_open_orders(self, symbol=None):
        Fetches all open orders for a given symbol.

    fetch_balance(self, params=None):
        Fetches the account balance.

    fetch_ticker(self, symbol, params=None):
        Fetches the current price ticker for a given symbol.

    place_market_order_with_depth(self, symbol, side, amount, depth_percent):
        Places a market order at a specified depth percentage below or above the current market price.

    drip_by_time(self, symbol, side, total_amount, chunks, pause_seconds):
        Places repeat market orders in chunks with a pause between each order.

    drip_to_limit(self, symbol, side, total_amount, chunks, pause_seconds, limit_price):
        Places repeat market orders in chunks with a pause, but temporarily stops if the price is below a set limit.

    drip_to_market(self, symbol, side, total_amount, chunks):
        Places repeat market orders in chunks, waiting for the market to respond before proceeding with the next order.

    iceberg_order(self, symbol, side, total_amount, iceberg_limit):
        Monitors price and places orders to maintain the iceberg limit until the total amount is exhausted.

    top_of_book(self, symbol, side, total_amount):
...
class Execution:
    ...
    def create_order(self, symbol, side, order_type, amount, price):
        """
    ...
    def cancel_orders(self, order_ids, symbol):
        """
        Cancel multiple open orders.
        """
        try:
            canceled_orders = self.exchange.cancel_orders(order_ids, symbol)
    ...
    def fetch_open_order(self, order_id, symbol=None):
        """
        Fetch details of an open order.
        """
        try:
            open_order = self.exchange.fetch_open_order(order_id, symbol)
            return open_order
        except Exception as e:
            return str(e)
    ...
    def fetch_ticker(self, symbol, params=None):
        """
        Fetch the price ticker for a given symbol.
        """
        try:
    ...
    def create_market_order(self, symbol, side, amount, depth_percent=50):
        """
        ...
        try:
            order = self.create_order(symbol, side, "limit", amount, adjusted_price)
            return order
        except Exception as e:
            return str(e)

    # Drip by time - places repeat market orders in chunks, with pause in between
    def drip_by_time(self, symbol, side, total_amount, chunks, pause_seconds):
        """
        ...
        def place_orders(self):
            for _ in range(chunks):
                if self.killswitch[0]:
                    break
                try:
                    order = self.create_market_order(symbol, side, chunk_size)
    ...
    def drip_to_limit(
        self, symbol, side, total_amount, chunks, pause_seconds, limit_price
    ):
        """
    ...
    def drip_to_market(self, symbol, side, total_amount, chunks):
        """
    ...
    def iceberg_order(self, symbol, side, total_amount, iceberg_limit):
        """
        ...
        def place_orders():
            while amount_remaining > 0 and not self.killswitch[0]:
                ...
                if (side == "buy" and price <= iceberg_limit) or (
                    ...
                    try:
                        order = self.create_market_order(symbol, side, order_size)
    ...
    def top_of_book(self, symbol, side, total_amount):
        """
        ...
        def place_order(self):
            while not self.killswitch[0]:
                ...
                price = ticker["ask"] if side == "buy" else ticker["bid"]
                try:
                    order = self.create_market_order(symbol, side, total_amount)
